...
try {
	...
	$backofficeKey = strpos($backofficeKey, '*') !== false ? GatewaySetting::getValue(Config::IFTHENPAYGATEWAY, Config::CF_BACKOFFICE_KEY) : $backofficeKey;
	...
	$body =  $smarty->fetch(ROOTDIR . '/modules/gateways/ifthenpaylib/lib/templates/mail/requestNewGatewayMethod.tpl');

	$message->setBodyAndPlainText($body);

	Mail::factory()->send($message);
	IfthenpayLog::info('general_logs', 'Request new gateway method email sent with success.');

} catch (\Throwable $th) {
	IfthenpayLog::error('general_logs', 'Failed to send new gateway method request email.' . $th->__toString());
	$responseData = [
		'success' => false,
