...
class PayshopRepository
{
	...
	public static function resetConfig(): void
	{
		Capsule::table('tblpaymentgateways')
			->where('gateway', Config::PAYSHOP_MODULE_CODE)
			->whereIn('setting', [
				Config::CF_BACKOFFICE_KEY,
				Config::CF_PAYSHOP_KEY,
				Config::CF_DEADLINE,
	...
	public static function savePayment(array $data): void
	{
		try {
			...
			$exists = Capsule::table(Config::PAYSHOP_TABLE)
			...
			if (!$exists) {
				...
				Capsule::table(Config::PAYSHOP_TABLE)
					->insert($data);
			} else {
			...
			IfthenpayLog::info(Config::PAYSHOP, 'Payment details saved successfully.', ['record_data' => $data]);
		} catch (\Throwable $th) {
			IfthenpayLog::error(Config::PAYSHOP, 'Unexpected error saving payment record', $th->__toString());
		}
	}
	...
	{
		$paymentData = Capsule::table(Config::PAYSHOP_TABLE)
	...
	{
		$payments = Capsule::table(Config::PAYSHOP_TABLE)
