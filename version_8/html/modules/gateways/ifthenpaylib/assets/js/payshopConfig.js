...
class PayshopConfig {
	 ...
	 */
	constructor(containerSelector = '#configifthenpaypayshop') {
	...
	async init() {
		...
		this.payshopContainerDom = document.querySelector(this.containerSelector) || null;
		if (!this.payshopContainerDom) return;

		this.backofficeKeyInputDom = this.payshopContainerDom.querySelector('input[name*="backofficeKey"]');
		if (!this.backofficeKeyInputDom) return;

		this.keySelectDom = this.payshopContainerDom.querySelector('select[name*="key"]') || null;
		if (!this.keySelectDom) return;

		this.deadlineInputDom = this.payshopContainerDom.querySelector('input[name*="deadline"]') || null;
		...
		Utils.reloadConfigPageIfSaveSuccessfull(this.payshopContainerDom);
	...
	async getKeys() {
		...
		try {
			const response = await fetch("../modules/gateways/ifthenpaylib/controllers/getkeys.php", {
				...
				body: new URLSearchParams({ 'backofficeKey': backofficeKey, 'paymentMethod': 'payshop'}),
			});
	...
	async showRequestAccountButton() {
		if (window.confirm(IftpLang.trans('msg_no_payshop_accounts_found') + '\n' + IftpLang.trans('msg_request_new_account'))) {
...
export function payshopConfig() {
	return new PayshopConfig();
}
