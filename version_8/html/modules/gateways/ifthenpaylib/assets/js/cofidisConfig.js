...
class CofidisConfig {
	 ...
	 */
	constructor(containerSelector = '#configifthenpaycofidis') {
	...
	async init() {
		...
		this.cofidisContainerDom = document.querySelector(this.containerSelector) || null;
		if (!this.cofidisContainerDom) return;

		this.backofficeKeyInputDom = this.cofidisContainerDom.querySelector('input[name*="backofficeKey"]');
		if (!this.backofficeKeyInputDom) return;

		this.keySelectDom = this.cofidisContainerDom.querySelector('select[name*="key"]') || null;
		if (!this.keySelectDom) return;

		this.minInputDom = this.cofidisContainerDom.querySelector('input[name*="min"]');
		if (!this.minInputDom) return;

		this.maxInputDom = this.cofidisContainerDom.querySelector('input[name*="max"]');
		if (!this.maxInputDom) return;
		...
		Utils.reloadConfigPageIfSaveSuccessfull(this.cofidisContainerDom);
	}
	...
	async updateMinMax() {
		...
		try {
			...
			const response = await fetch("../modules/gateways/ifthenpaylib/controllers/getcofidisminmax.php", {
				method: "POST",
				headers: {
			...
			if (result.success) {
				this.minInputDom.value = result.data.minAmount;
				this.maxInputDom.value = result.data.maxAmount;

			} else {
				console.error("Error: unable to fetch min max.");
			}
		} catch (error) {
			console.error("Error: unable to fetch min max.");
		} finally {
	...
	async getKeys() {
		...
		try {
			const response = await fetch("../modules/gateways/ifthenpaylib/controllers/getkeys.php", {
				...
				body: new URLSearchParams({ 'backofficeKey': backofficeKey, 'paymentMethod': 'cofidis' }),
			});
	...
	async showRequestAccountButton() {
		if (window.confirm(IftpLang.trans('msg_no_cofidis_accounts_found') + '\n' + IftpLang.trans('msg_request_new_account'))) {
...
export function cofidisConfig() {
	return new CofidisConfig();
}
