...
class PixConfig {
	 ...
	 */
	constructor(containerSelector = '#configifthenpaypix') {
	...
	async init() {
		...
		this.pixContainerDom = document.querySelector(this.containerSelector) || null;
		if (!this.pixContainerDom) return;

		this.backofficeKeyInputDom = this.pixContainerDom.querySelector('input[name*="backofficeKey"]');
		if (!this.backofficeKeyInputDom) return;

		this.keySelectDom = this.pixContainerDom.querySelector('select[name*="key"]') || null;
		...
		Utils.reloadConfigPageIfSaveSuccessfull(this.pixContainerDom);
	...
	async getKeys() {
		...
		try {
			const response = await fetch("../modules/gateways/ifthenpaylib/controllers/getkeys.php", {
				...
				body: new URLSearchParams({ 'backofficeKey': backofficeKey, 'paymentMethod': 'pix'}),
			});
	...
	async showRequestAccountButton() {
		if (window.confirm(IftpLang.trans('msg_no_pix_accounts_found') + '\n' + IftpLang.trans('msg_request_new_account'))) {
