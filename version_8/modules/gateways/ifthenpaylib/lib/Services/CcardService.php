...
class CcardService
{
	...
	public static function validateCallback(array $request, array $settings)
	{
		...
		if (empty($invoice)) {
			IfthenpayLog::info(Config::CCARD, 'validateCallback - Invoice not found. ERROR code: ' . Config::CB_ERROR_INVOICE_NOT_FOUND, ['request' => $request, 'storedData' => $storedData]);
		...
		if (round($invoiceAmount, 2) !== round($requestAmount, 2)) {
			IfthenpayLog::info(Config::CCARD, 'validateCallback - Invalid amount. ERROR code: ' . Config::CB_ERROR_INVALID_AMOUNT, ['request' => $request, 'invoiceAmount' => $invoiceAmount]);
