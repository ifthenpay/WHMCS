...
class PayshopService
{
	...
	{
		try {
			$backofficeKey = GatewaySetting::getValue(Config::PAYSHOP_MODULE_CODE, Config::CF_BACKOFFICE_KEY) ?? '';
			...
			IfthenpayLog::info(Config::PAYSHOP, 'Accounts refreshed successfully.', ['accounts' => $accounts]);

			return true;
		} catch (\Throwable $th) {
			IfthenpayLog::error(Config::PAYSHOP, 'Unexpected error refreshing accounts', $th->__toString());
			return false;
		}
	}
	...
	public static function validateCallback(array $request, array $settings)
	{
		...
		if (empty($invoice)) {
			IfthenpayLog::info(Config::PAYSHOP, 'validateCallback - Invoice not found. ERROR code: ' . Config::CB_ERROR_INVOICE_NOT_FOUND, ['request' => $request, 'storedData' => $storedData]);
		...
		if (round($invoiceAmount, 2) !== round($requestAmount, 2)) {
			IfthenpayLog::info(Config::PAYSHOP, 'validateCallback - Invalid amount. ERROR code: ' . Config::CB_ERROR_INVALID_AMOUNT, ['request' => $request, 'invoiceAmount' => $invoiceAmount]);
