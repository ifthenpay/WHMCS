...
class MbwayRepository
{
	...
	public static function resetConfig(): void
	{
		Capsule::table('tblpaymentgateways')
			->where('gateway', Config::MBWAY_MODULE_CODE)
			->whereIn('setting', [
				Config::CF_MBWAY_KEY,
				...
				Config::CF_MBWAY_NOTIFICATION_DESCRIPTION,
				Config::CF_MBWAY_SHOW_COUNTDOWN,
	...
	public static function savePayment(array $data): void
	{
		try {
			...
			$exists = Capsule::table(Config::MBWAY_TABLE)
			...
			if (!$exists) {
				...
				Capsule::table(Config::MBWAY_TABLE)
					->insert($data);
			} else {
			...
			IfthenpayLog::info(Config::MBWAY, 'Payment details saved successfully.', ['record_data' => $data]);
		} catch (\Throwable $th) {
			IfthenpayLog::error(Config::MBWAY, 'Unexpected error saving payment record', $th->__toString());
		}
	}
	...
	{
		$paymentData = Capsule::table(Config::MBWAY_TABLE)
	...
	{
		$payments = Capsule::table(Config::MBWAY_TABLE)
