...
class CcardRepository
{
	...
	public static function resetConfig(): void
	{
		Capsule::table('tblpaymentgateways')
			->where('gateway', Config::CCARD_MODULE_CODE)
			->whereIn('setting', [
	...
	public static function savePayment(array $paymentData): void
	{
		try {
			...
			$exists = Capsule::table(Config::CCARD_TABLE)
			...
			if (!$exists) {
				...
				Capsule::table(Config::CCARD_TABLE)
					->insert($data);
			} else {
			...
			IfthenpayLog::info(Config::CCARD, 'Payment details saved successfully.', ['record_data' => $data]);
		} catch (\Throwable $th) {
			IfthenpayLog::error(Config::CCARD, 'Unexpected error saving payment record', $th);
		}
	}
	...
	{
		$paymentData = Capsule::table(Config::CCARD_TABLE)
	...
	{
		$payments = Capsule::table(Config::CCARD_TABLE)
