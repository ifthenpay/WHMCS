...
abstract class ConfigForm
{
	...
	public function __construct(
		...
		$this->form = $this->ioc->makeWith(
			Form::class,
			['paymentMethod' => $this->paymentMethod, 'paymentMethodNameAlias' => method_exists($this, 'getPaymentMethodNameAlias') ? $this->getPaymentMethodNameAlias() : $this->paymentMethodNameAlias]
		);
	...
	private function addCallbackInfoToConfigForm(): void
	{
		if ($this->activatedCallback) {
			$btn = '<span class="badge ifthenpay_callback_badge green" >' . \AdminLang::trans('callbackActivated') . '</span>';
		} else {
			$btn = '<span class="badge ifthenpay_callback_badge red" data-toggle="tooltip" data-placement="top" title="' . \AdminLang::trans('callbackNotActivatedTooltip') . '">' . \AdminLang::trans('callbackNotActivated') . '</span>';
		}
	...
	protected function addUpgradeModuleToForm(): void
	{
		try {
			...
			$data = [
				...
				'ifthenpayModuleVersion' => $this->ifthenpayUpgrade::MODULE_VERSION
			];
	...
	protected function addToOptions(bool $afterSave = false): void
	{
		...
		$this->ifthenpayLogger->info(
			'ifthenpay account options retrieved with success',
			[
	...
	protected function saveCallback(string $url, string $antiPhishingKey, bool $activateCallback)
	{
		try {
			...
			$this->ifthenpayLogger->info(
				'callback activation data saved with success',
				[
			...
			$this->ifthenpayLogger->error(
				'error saving callback activation data in database',
				[
					'paymentMethod' => $this->paymentMethod,
					'url' => $url,
					'antiPhishingKey' => $antiPhishingKey,
	...
	public function processForm(): void
	{
		try {
			if (!$this->activatedCallback && !empty($this->configValues)) {
				...
				$this->ifthenpayLogger->info(
					'callback data build with success',
					[
						'activateCallback' => $activateCallback,
